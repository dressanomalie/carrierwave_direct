...
module CarrierWaveDirect
  module Policies
    class Aws4HmacSha256 < Base
      ...
      def credential
        "#{uploader.access_key_id}/#{timestamp.strftime("%Y%m%d")}/#{uploader.region}/s3/aws4_request"
      end
      ...
      def signing_key(options = {})
        ...
        kDate    = OpenSSL::HMAC.digest('sha256', "AWS4" + uploader.secret_access_key, timestamp.strftime("%Y%m%d"))
